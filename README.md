...
純前端 PWA（vanilla HTML/CSS/JS，無建置流程），資料存在裝置本機的 IndexedDB。照片標註與 PDF 報告完全在裝置端產生，不會經過任何伺服器。選配 OneDrive 雲端同步，讓你在手機與電腦之間共用同一份工地資料。設定頁也可匯出／匯入含照片的完整 JSON 備份。
...
第三方前端程式庫已固定版本並放在 `vendor/`，因此安裝 PWA 後，登入與 PDF 匯出不依賴 CDN 連線。
